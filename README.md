...
The project will consist of two main components. The first is audio preprocessing, in which the input track is divided into individual stems. The second is visualization within UE, where the separated audio components are automatically imported via script and individually mapped to Niagara particle effects based on audio features such as beat, frequency, and amplitude. Particle behaviours will be preconfigured so that when new audio tracks are introduced into the pipeline, the visualisation responds in real time. Each instrument will have a dedicated particle system that can be toggled on or off by the user. This approach enables the creation of a complex system that visually represents the sounds perceived when listening to a given melody. 
...
## Literature review
Many recent works show that deep learning based music source separation on MUSDB18 is a mature and widely adopted approach, which supports our choice to utilize a pre-trained model Spleeter as a baseline [2], [3]. Spleeter provides pretrained 2, 4, and 5 stem U-net models trained on large multitrack collections and evaluated on MUSDB18, achieving high quality of separation while running up to 100 times faster than real time on a single GPU, which makes it well suited for offline stem generation that can be imported into Unreal Engine [4], [5]. 

Documentation for Unreal Engine demonstrates how Niagara and the Audio Synesthesia system can drive particle attributes such as spawn rate, size, and colour from per-band amplitude or submix data [6], [7], [8]. In addition, we will also consider Demucs as an alternative model, since it has been shown to outperform many spectrogram-based systems [9]. Demucs is a waveform-to-waveform U-net model, achieves a higher average of SDR than previous architectures on MUSDB18. Comparing stems produced by Spleeter and Demucs on the same tracks will allow us to study the trade-offs of quality and efficiency.

Moreover, from a perceptual perspective, relationships between sound and colours has been an actively explored topic since 1704 [10]. For example, multiple studies attempt to map colour parameters such as hue, saturation and luminance to properties of sound such as pitch, loudness and noisiness [11], [12]. Furthermore, Vickery L. suggests that certain instruments can be represented by specific ranges of colour [11]. While pure sound waves demonstrate relatively stable cross-mappings, visually representing more complex sounds remains significantly more challenging [11]. Despite the large amount of published research, there is still no existing one-to-one mapping between sound and colour, as perception varies between individuals and is strongly influenced by the emotional aspects [13], [14]. Therefore, our implementation will not follow a fixed mapping chart, but instead draws inspiration from existing studies.


## Required Tools 
We plan to start with Python’s Spleeter library [4] as it offers a proven architecture for music source separation and significantly reduces implementation complexity. Additionally, we intend to complete the same separation process using the pre-trained audio source separation model Demucs [15]. Our main objective is to compare these two approaches in terms of separation quality and computational cost, and to select the model that best fits the system’s requirements.

This approach helps address the difficulty of visually representing complex musical structures identified through Literature Review. By separating a song into individual components, the system can represent different elements of the audio more accurately. 
...
[9]	A. Défossez, N. Usunier, L. Bottou, and F. Bach, “Music Source Separation in the Waveform Domain,” arXiv.org. Accessed: Feb. 10, 2026. [Online]. Available: https://arxiv.org/abs/1911.13254v2 

[10]	I. Newton, Opticks. London : Printed for Sam Smith, and Benj. Walford, printers to the Royal Society, at the prince’s Arms in St. paul’s Church-yard, 1704. Accessed: Feb. 09, 2026. [Online]. Available: http://archive.org/details/opticksortreatis00newt 

[11]	L. Vickery, “Some approaches to representing sound with colour and shape,” Res. Outputs 2014 2021, Jan. 2018, [Online]. Available: https://ro.ecu.edu.au/ecuworkspost2013/6410 

[12]	G. Hamilton-Fletcher, C. Witzel, D. Reby, and J. Ward, “Sound Properties Associated With Equiluminant Colours,” Jan. 2017, doi: 10.1163/22134808-00002567. 

[13]	C. Spence and N. Di Stefano, “Coloured hearing, colour music, colour organs, and the search for perceptually meaningful correspondences between colour and sound,” -Percept., vol. 13, no. 3, p. 20416695221092802, May 2022, doi: 10.1177/20416695221092802. 

[14]	L. Wilms and D. Oberfeld, “Color and emotion: effects of hue, saturation, and brightness,” Psychol. Res., vol. 82, no. 5, pp. 896–914, Sep. 2018, doi: 10.1007/s00426-017-0880-8. 

[15]	S. Kim, “Music Source Separation with Hybrid Demucs — Torchaudio 2.10.0 documentation.” Accessed: Feb. 10, 2026. [Online]. Available: https://docs.pytorch.org/audio/stable/tutorials/hybrid_demucs_tutorial.html?utm_ 
